...
function previousMinimumElement(arr, n){
    ...
    for(let i=0; i<n; i++){         //for each element  going through the loop
        ...
        if(st.isEmpty()){
            prevMin[i] = -1;           //no previous  minimum element so  -1
        }
        else{
            prevMin[i] = st.peek();   //popping top element out as that will be previous minimum
        }
        st.push(i);
    }
    return prevMin;
...
function nextMinimumElement(arr, n){
    ...
    for(let i=n-1; i>=0; i--){           // this checks from RHS
