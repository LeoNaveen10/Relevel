/**
 * class notes on first unique elements
 * 
 */
...
function firstUniqueChar(str){
    ...
    let freqArr = new Array(26);   //creating frequncy array and incrementing it for every character
    freqArr.fill(0);
    let uniqueElementsQueue = new Queue();

    for(let i=0; i<str.length; i++){
        let ch = str[i];
        let indexOfCh = ch.charCodeAt(0) - 'a'.charCodeAt(0);      //storing as per ASCII values

        freqArr[indexOfCh]++; //increase the frequency of current char ch

        if(freqArr[indexOfCh]==1){ //if unique enque
            uniqueElementsQueue.enqueue(ch);

        }else{
            while(!uniqueElementsQueue.isEmpty() &&   //not unique dequeue
            freqArr[uniqueElementsQueue.getFront().charCodeAt(0) - 'a'.charCodeAt(0)] >1){
                uniqueElementsQueue.dequeue();
            }
        }

        if(uniqueElementsQueue.isEmpty()){    //for every charcter we are printing results
